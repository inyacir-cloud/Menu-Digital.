...
import { HelpIcon, PlusIcon, TacoIcon } from "./icons";
...
export function ComboBand({ combos, closed, qtyOf, onOpen }: Props) {
  ...
  return (
    <section id="combos" className="scroll-mt-24" aria-labelledby="combos-title">
      <div className="rounded-[1.75rem] border-2 border-dashed border-emerald-300 bg-emerald-50/80 px-4 py-5 sm:px-6 sm:py-6">
        <header className="flex flex-wrap items-center gap-3">
          <span className="grid h-11 w-11 shrink-0 place-items-center rounded-full bg-emerald-500 text-white shadow-sm">
            <TacoIcon className="h-5 w-5" />
          </span>
          <div>
            <h2 id="combos-title" className="text-[1.9rem] font-bold tracking-tight text-ink sm:text-4xl">Combos</h2>
            <span className="mt-1 block h-1 w-10 rounded-full bg-emerald-500" />
          </div>
          <span className="inline-flex items-center gap-1.5 rounded-full border border-emerald-300 bg-white/70 px-3 py-1.5 text-xs font-semibold text-ink/80">
            <HelpIcon className="h-3.5 w-3.5 text-emerald-600" /> Elige tus favoritos
          </span>
        </header>
            ...
            return (
              <li key={combo.id} className="flex items-center gap-3 rounded-xl px-2 py-2 transition-colors hover:bg-white/60">
                <button type="button" onClick={() => onOpen(combo)} className="min-w-0 flex-1 text-left">
                  ...
                  {combo.badge && <span className="text-xs font-bold uppercase tracking-wider text-emerald-700">{combo.badge}</span>}
                </button>
