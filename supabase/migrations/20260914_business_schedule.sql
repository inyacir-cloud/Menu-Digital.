-- Horario estructurado del negocio para apertura automática o control manual.
alter table public.settings
  add column if not exists schedule_mode text not null default 'automatic',
  add column if not exists schedule jsonb;

alter table public.settings
  drop constraint if exists settings_schedule_mode_check;

alter table public.settings
  add constraint settings_schedule_mode_check
